...
fn radixSortC(
    ...
    if(gl_LocalInvocationID.x == 0u && global_entry_offset + #{WORKGROUP_ENTRIES_C} >= arrayLength(&points)) {
    ...
    if(sorting_pass_index == #{RADIX_DIGIT_PLACES} - 1u && gl_LocalInvocationID.x == #{WORKGROUP_INVOCATIONS_C} - 2u && global_entry_offset + #{WORKGROUP_ENTRIES_C} >= arrayLength(&points)) {
