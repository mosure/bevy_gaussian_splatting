...
pub fn stream_ply_3d(
    ...
    for (_, element) in &header.elements {
        if element.name == "vertex" {
            validate_gaussian_3d_properties(element)?;
    ...
    for (_, element) in &header.elements {
        for element_index in 0..element.count {
            let mut value = read_ply_element(&parser, &mut reader, element, header.encoding)?;
            if element.name != "vertex" {
                continue;
            }
...
fn validate_gaussian_3d_properties(element: &ElementDef) -> io::Result<()> {
    ...
    if let Some(malformed) = element
        .properties
        .keys()
        .find(|name| name.starts_with("f_rest_") && name[7..].parse::<usize>().is_err())
    {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!("malformed Gaussian spherical-harmonic property '{malformed}'"),
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    fn rest_coefficients_use_standard_channel_major_ply_layout() {
        ...
        assert_eq!(
            gaussian.spherical_harmonic.coefficients[SH_CHANNELS + 1],
            20.0
        );
    }
